...
library DotnsConstantsOld {
    ...
    ///      `DotnsPopControllerOld` and `DotnsNameWhitelistOld` to authenticate
    ...
    /// @dev Consumed by `PopRulesOld` to read each account's personhood tier
    ...
    /// @dev Role: single authority for the wei amount a registration costs. `PopRulesOld` resolves
    ///      it here on every pricing read, so swapping the model is one `set` on the protocol
    ///      registry without redeploying `PopRulesOld` or its consumers.
    /// forge-lint: disable-next-line(unsafe-typecast)
    ...
    ///      `DotnsNameEscrowOld.MAX_REDEEM_WINDOW`. Live deployments rotate the runtime value via
    ///      `updateRedeemWindow`.
    ...
    ///      two can coexist per `DotnsRegistrarOld`'s multi-controller affordance.
    /// forge-lint: disable-next-line(unsafe-typecast)
    ...
    ///      resolver, and PopRulesOld. Holds no authority and is consumed by clients, not by other
    ///      contracts.
    /// forge-lint: disable-next-line(unsafe-typecast)
