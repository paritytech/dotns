...
import {StoreUtilsOld} from "./StoreUtilsOld.sol";
...
library RegistrationUtilsOld {
    ...
    ///      so this helper deliberately does not call `StoreUtilsOld.writeLabel`.
    ...
    function _resolveSiblings(IDotnsProtocolRegistryOld protocolRegistry)
        private
        view
        returns (Siblings memory siblings)
    {
        siblings = Siblings({
            ...
            registry: IDotnsRegistryOld(protocolRegistry.get(DotnsConstantsOld.REGISTRY)),
            storeFactory: IStoreFactoryOld(protocolRegistry.get(DotnsConstantsOld.STORE_FACTORY))
        });
    }
}
