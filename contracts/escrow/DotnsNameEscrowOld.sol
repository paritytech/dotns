    ...
    IDotnsNameEscrow
{
    ...
    function release(uint256 tokenId) external override nonReentrant {
        IDotnsRegistrarOld registrar = _registrar();
    ...
    function _registrar() internal view returns (IDotnsRegistrarOld registrar) {
    ...
    function _onlyController() internal view {
        address controller = protocolRegistry.get(DotnsConstantsOld.CONTROLLER);
        require(msg.sender == controller, NotController(msg.sender));
    }

    /// @notice Restricts calls to the configured registrar from the protocol registry.
    function _onlyRegistrar() internal view {
        address registrar = protocolRegistry.get(DotnsConstantsOld.REGISTRAR);
