...
/// @dev Pre-upgrade snapshot of the controller interface, pinned so the layout diff has a
/// compilable predecessor for the public surface. Defines allocation only; forward resolution,
/// reverse lookup, pricing mechanics, PoP validation, and store writing are handled by external
/// contracts. Users commit a hash of registration parameters and, after a minimum delay, reveal
/// the same parameters to register. Implementations write the successfully registered name into
/// the user's Store to create an immutable on-chain record that doubles as a quick lookup for all
/// names registered.
...
interface IDotnsRegistrarControllerOld is IDotnsController {
    ...
    /// any other caller reverts with @custom:reverts IDotnsRoleManagerOld.NotRoleOrOwner. Emits
