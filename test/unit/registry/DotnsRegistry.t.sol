...
contract DotnsRegistryTests is BaseDotns {
    ...
    function test_setApprovalForAll_records_and_reports_the_grant() public {
        vm.expectEmit(true, true, false, true, address(dotnsRegistry));
        emit IDotnsRegistry.ApprovalForAll(ed, leonardo, true);

        vm.prank(ed);
        dotnsRegistry.setApprovalForAll(leonardo, true);

        assertTrue(dotnsRegistry.isApprovedForAll(ed, leonardo), "grant recorded");
        assertFalse(dotnsRegistry.isApprovedForAll(ed, tiago), "unrelated operator not granted");
        assertFalse(dotnsRegistry.isApprovedForAll(leonardo, ed), "grant is directional");
    }

    function test_registry_operator_is_authorised_for_tokenised_node() public {
        bytes32 node = _register("operatorbase1", ed, IPopRules.PopStatus.NoStatus);
        assertFalse(dotnsRegistry.isAuthorised(node, leonardo), "not authorised before grant");

        vm.prank(ed);
        dotnsRegistry.setApprovalForAll(leonardo, true);

        assertTrue(dotnsRegistry.isAuthorised(node, leonardo), "operator authorised for owner node");

        address chosenResolver = makeAddr("chosenResolver");
        vm.prank(leonardo);
        dotnsRegistry.setResolver(node, chosenResolver);
        assertEq(dotnsRegistry.resolver(node), chosenResolver, "operator may set the resolver");
    }

    function test_registry_operator_is_authorised_for_subnode() public {
        string memory parentLabel = "operatorbase2";
        bytes32 parentNode = _register(parentLabel, ed, IPopRules.PopStatus.NoStatus);

        bytes32 subnode = _namehash(parentNode, keccak256(bytes("sub")));
        vm.prank(ed);
        dotnsRegistry.setSubnodeOwner(
            IDotnsRegistry.SubnodeRecord({
                parentNode: parentNode, subLabel: "sub", parentLabel: parentLabel, owner: ed
            })
        );

        assertFalse(dotnsRegistry.isAuthorised(subnode, tiago), "not authorised before grant");

        vm.prank(ed);
        dotnsRegistry.setApprovalForAll(tiago, true);

        assertTrue(dotnsRegistry.isAuthorised(subnode, tiago), "operator authorised for subnode");
    }

    function test_revoked_registry_operator_loses_authority() public {
        bytes32 node = _register("operatorbase3", ed, IPopRules.PopStatus.NoStatus);

        vm.prank(ed);
        dotnsRegistry.setApprovalForAll(leonardo, true);
        assertTrue(dotnsRegistry.isAuthorised(node, leonardo), "authorised while granted");

        vm.prank(ed);
        dotnsRegistry.setApprovalForAll(leonardo, false);
        assertFalse(dotnsRegistry.isAuthorised(node, leonardo), "authority withdrawn on revoke");

        vm.prank(leonardo);
        vm.expectRevert(IDotnsRegistry.NotAuthorised.selector);
        dotnsRegistry.setResolver(node, makeAddr("resolver"));
    }

    function test_registrar_operator_is_not_registry_authorised() public {
        bytes32 node = _register("operatorbase4", ed, IPopRules.PopStatus.NoStatus);

        // A registrar transfer approval must not bridge into registry record authority.
        vm.prank(ed);
        dotnsRegistrar.setApprovalForAll(leonardo, true);
        assertTrue(
            dotnsRegistrar.isApprovedForAll(ed, leonardo), "registrar grant is active for the guard"
        );

        assertFalse(
            dotnsRegistry.isAuthorised(node, leonardo),
            "registrar transfer approval confers no record authority"
        );
    }
}
