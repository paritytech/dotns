...
contract DotnsRegistrarControllerLifecycleTest is BaseDotns {
    function test_register_reclaim_succeeds_for_new_poplite_owner() public {
        ...
        assertEq(
            reservationOwner,
            newOwner,
            "stale reservation must not block reclaim"
        );
    }

    function test_register_cross_payer_charges_max_not_sum_of_price_and_reach()
        public
    {
        ...
        assertGt(
            friction,
            0,
            "scenario requires non-zero payer-to-owner friction"
        );
    ...
    function test_revert_register_cross_payer_when_msg_value_below_max_of_price_and_reach()
        public
    {
        string memory label = NOSTATUS_LABEL_A;
        address payer = leonardo;
        ...
        IDotnsRegistrarController.Registration
            memory registration = IDotnsRegistrarController.Registration({
                ...
                owner: nameOwner,
                secret: keccak256(
                    abi.encodePacked(label, nameOwner, payer, "short")
                ),
                reserved: true
            });
        ...
        dotnsRegistrarController.register{value: expectedCharge - 1}(
            registration
        );
    }

    function test_register_cross_payer_routes_owner_price_to_insurance()
        public
    {
        string memory label = NOSTATUS_LABEL_A;
        address payer = leonardo;
        ...
        IDotnsRegistrarController.Registration
            memory registration = IDotnsRegistrarController.Registration({
                ...
                owner: nameOwner,
                secret: keccak256(
                    abi.encodePacked(label, nameOwner, payer, "ins")
                ),
                reserved: true
            });
        ...
        assertEq(
            position.amount,
            0,
            "cross-payer must not seed a refundable position"
        );
        assertEq(
            dotnsNameEscrow.insuranceFund() - insuranceBefore,
            ownerPrice,
    ...
    function test_register_creates_escrow_position_for_zero_priced_registration()
        public
    {
        ...
        IDotnsRegistrarController.Registration
            memory registration = IDotnsRegistrarController.Registration({
                ...
                owner: nameOwner,
                secret: keccak256(
                    abi.encodePacked(label, nameOwner, "popfull")
                ),
                reserved: true
            });
        bytes32 commitment = dotnsRegistrarController.makeCommitment(
        ...
        assertEq(
            atMint.recipient,
            nameOwner,
            "position must bind the registrant at mint"
        );
        assertEq(
            atMint.amount,
            0,
        ...
        IDotnsNameEscrow.ReleasePosition memory atRelease = dotnsNameEscrow
            .getReleasePosition(tokenId);
        assertTrue(
            atRelease.released,
            "zero-priced registration must still be releasable"
        );
        assertEq(
            atRelease.recipient,
            nameOwner,
            "release recipient must be the registrant"
        );
    }

    function test_register_reclaim_state_consistent_during_safe_transfer_callback()
        public
    {
        string memory label = NOSTATUS_LABEL_A;
        ...
        RegistrationProbe probe = new RegistrationProbe(
        ...
        uint256 ownerPrice = popRules
            .priceWithCheck(label, address(probe))
            .price;
        vm.prank(address(probe));
        dotnsRegistrarController.register{value: ownerPrice}(registration);
        ...
        assertTrue(
            probe.callbackFired(),
            "reclaim must run through onERC721Received"
        );
    ...
    function test_revert_registerreserved_for_already_seeded_label() public {
        ...
        vm.startPrank(owner);
        bytes32 secondCommitment = dotnsRegistrarController.makeCommitment(
            secondRegistration
        );
    ...
    function test_register_second_reserved_name_preserves_prior_primary_reverse_record()
        public
    {
    ...
    function test_register_overpayment_pushed_directly_to_eoa_payer() public {
        ...
        emit IDotnsRegistrarController.OverpaymentRefunded(
            registrant,
            overpayment
        );

        vm.prank(registrant);
        dotnsRegistrarController.register{value: ownerPrice + overpayment}(
            registration
        );
    ...
    function test_register_overpayment_pushed_directly_to_accepting_contract()
        public
    {
        string memory label = NOSTATUS_LABEL_A;
        ...
        uint256 ownerPrice = popRules
            .priceWithCheck(label, address(receiver))
            .price;
        uint256 overpayment = 1 ether;
    ...
    function test_register_overpayment_falls_back_to_ledger_on_rejecting_contract()
        public
    {
        string memory label = NOSTATUS_LABEL_A;
        ...
        emit IDotnsNameEscrow.OverpaymentRefunded(
            address(receiver),
            overpayment
        );
    ...
    function test_register_overpayment_reentrant_attacker_falls_back_to_ledger()
        public
    {
        string memory label = NOSTATUS_LABEL_A;

        ReentrantOverpaymentAttacker attacker = new ReentrantOverpaymentAttacker(
        ...
        IDotnsRegistrarController.Registration
            memory replay = IDotnsRegistrarController.Registration({
                label: replayLabel,
        ...
        uint256 ownerPrice = popRules
            .priceWithCheck(label, address(attacker))
            .price;
        uint256 overpayment = 1 ether;
    ...
    function test_revert_register_on_reentry_from_onerc721received() public {
        ...
        bytes32 replayCommitment = dotnsRegistrarController.makeCommitment(
            replay
        );
        vm.prank(address(attacker));
        ...
        IDotnsRegistrarController.Registration
            memory outer = IDotnsRegistrarController.Registration({
                label: label,
                owner: address(attacker),
                secret: keccak256("outer-reclaim"),
                reserved: true
            });
        bytes32 outerCommitment = dotnsRegistrarController.makeCommitment(
            outer
        );
        vm.prank(address(attacker));
