...
contract DotnsPopControllerTests is BaseDotns {
    function test_reserveBaseName_mints_and_wires_registry_and_resolver()
        public
    {
    ...
    function test_reserveBaseName_enqueues_when_reserved_label_provided()
        public
    {
        ...
        (bool reserved, address holder) = dotnsPopController.isReservedForClaim(
            BASE_LABEL_A
        );
        assertTrue(reserved);
        assertEq(holder, ed);
    ...
    function test_registerBaseName_claim_emits_claim_event_and_not_standalone()
        public
    {
        ...
        _assertEventEmittedOnce(
            logs,
            keccak256("BaseNameClaimed(bytes32,address,string)")
        );
    ...
    function test_registerBaseName_standalone_emits_standalone_event_and_not_claim()
        public
    {
        _grantPopFull(tiago);
    ...
    function test_registerBaseName_claim_inherits_chat_key_from_lite_node()
        public
    {
        ...
        assertEq(
            dotnsPopResolver.liteLink(fullNode),
            keccak256(bytes(LITE_LABEL_A))
        );
    }
    ...
    function test_registerBaseName_claim_wipes_entire_queue() public {
        ...
        (, address wonderHolder) = dotnsPopController.isReservedForClaim(
            BASE_LABEL_B
        );
        assertEq(wonderHolder, tiago);
    }

    function test_registerBaseName_standalone_auto_relinquishes_users_other_reservation()
        public
    {
        _grantPopFull(ed);
        _reservePop(ed, LITE_LABEL_A, _validChatKey(0x01), BASE_LABEL_A);
    ...
    function test_registerBaseName_standalone_with_lite_link_silently_relinquishes()
        public
    {
        _grantPopFull(ed);
        _reservePop(ed, LITE_LABEL_A, _validChatKey(0x01), BASE_LABEL_A);
        ...
        _assertEventEmittedOnce(
            logs,
            keccak256("LiteToFullLinked(bytes32,bytes32)")
        );
        _assertEventNotEmitted(
            logs,
            keccak256("BaseNameClaimed(bytes32,address,string)")
        ...
        _assertEventNotEmitted(
            logs,
            keccak256("ReservationRelinquished(bytes32,address)")
        );
    ...
    function test_relinquishReservation_promotes_next_waiter_when_head_leaves()
        public
    {
        _grantPopFull(ed);
        _grantPopFull(tiago);
    ...
    function test_relinquishReservation_reverts_when_caller_has_no_reservation()
        public
    {
        vm.prank(ed);
        vm.expectRevert(
            abi.encodeWithSelector(
                IDotnsPopController.NoActiveReservation.selector,
                ed
            )
        );
        dotnsPopController.relinquishReservation();
    ...
    function test_setReservationDuration_shortening_retroactively_expires_live_entries()
        public
    {
        _grantPopFull(ed);
        ...
        (bool liveBefore, address holderBefore) = dotnsPopController
            .isReservedForClaim(BASE_LABEL_A);
        ...
        (bool liveAfter, ) = dotnsPopController.isReservedForClaim(
            BASE_LABEL_A
        );
    ...
    function test_setReservationDuration_reverts_for_non_owner() public {
        vm.prank(ed);
        vm.expectRevert(
            abi.encodeWithSignature("OwnableUnauthorizedAccount(address)", ed)
        );
        dotnsPopController.setReservationDuration(14 days);
    }

    function test_enqueueReservation_same_user_second_call_replaces_first()
        public
    {
        ...
        IDotnsPopController.UserReservation
            memory firstReservation = dotnsPopController.userReservation(ed);
        ...
        IDotnsPopController.UserReservation
            memory secondReservation = dotnsPopController.userReservation(ed);
        assertEq(secondReservation.labelhash, keccak256(bytes(BASE_LABEL_B)));

        (bool firstReserved, ) = dotnsPopController.isReservedForClaim(
            BASE_LABEL_A
        );
        assertFalse(firstReserved);

        (bool secondReserved, address secondHolder) = dotnsPopController
            .isReservedForClaim(BASE_LABEL_B);
        assertTrue(secondReserved);
        assertEq(secondHolder, ed);
    }

    function test_reEnqueue_after_own_expiry_promotes_same_user_to_head()
        public
    {
        ...
        (bool expiredReserved, ) = dotnsPopController.isReservedForClaim(
            baseStem
        );
        ...
        (bool nowReserved, address holder) = dotnsPopController
            .isReservedForClaim(baseStem);
    ...
    function test_claim_then_reEnqueue_on_same_stem_resets_cleanly() public {
        ...
        (bool oldSlot, address oldHolder) = dotnsPopController
            .isReservedForClaim(baseStem);
        assertFalse(oldSlot);
        ...
        (bool newSlot, address newHolder) = dotnsPopController
            .isReservedForClaim(BASE_LABEL_B);
        assertTrue(newSlot);
    ...
    function test_same_stem_lite_and_base_occupy_distinct_registrar_tokens()
        public
    {
        _grantPopFull(ed);
        ...
        assertEq(
            IERC721(address(dotnsRegistrar)).ownerOf(
                uint256(_nodeOf(LITE_LABEL_A))
            ),
            ed
        );
        assertEq(
            IERC721(address(dotnsRegistrar)).ownerOf(
                uint256(_nodeOf("aliceli"))
            ),
            tiago
        );
    }
    ...
    function test_both_controllers_can_mint_on_shared_registrar() public {
        ...
        assertEq(
            IERC721(address(dotnsRegistrar)).ownerOf(
                uint256(_nodeOf("longnamebob01"))
            ),
            tiago
        );
    }

    function test_gateway_reserved_name_rejects_public_register_by_other_user()
        public
    {
        _grantPopFull(tiago);
        ...
        bytes32 secret = keccak256(
            abi.encodePacked(label, ed, block.timestamp)
        );
    ...
    function test_gateway_reserved_name_allows_holder_to_register_via_public()
        public
    {
        _grantPopFull(tiago);
        _reservePop(tiago, LITE_LABEL_A, _validChatKey(0x11), "longnamebob");
    ...
    function test_second_pop_lite_mint_of_same_label_reverts_at_registrar()
        public
    {
        _grantPopFull(ed);
        _reservePop(ed, LITE_LABEL_A, _validChatKey(0xaa), "");
    ...
    function test_public_register_after_pop_full_mint_reverts_at_registrar()
        public
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IDotnsRegistrarController.NameNotAvailable.selector,
                "longnamebob01"
            )
        );
    ...
    function test_non_owner_cannot_create_subname_under_pop_minted_name()
        public
    {
        _grantPopFull(ed);
        IDotnsPopController.Link memory link = _linkFresh(_validChatKey(0xcf));
    ...
    function test_pop_reservation_of_already_public_minted_name_reverts_at_reserve_time()
        public
    {
        ...
        (bool reserved, ) = dotnsPopController.isReservedForClaim(
            "longnamebob"
        );
        assertFalse(reserved);
    }
    ...
    function test_enqueue_becomesHead_writes_popRules_reservation() public {
        ...
        (address holder, uint64 expires) = popRules.getBaseNameReservation(
            "longnamebob"
        );
        assertEq(holder, ed);
        assertEq(
            expires,
            uint64(block.timestamp + popRules.MAX_RESERVATION_TIME())
        );
    }
    ...
    function test_advanceExpiredHead_last_expire_releases_popRules_slot()
        public
    {
        _grantPopFull(ed);
        _reservePop(ed, LITE_LABEL_A, _validChatKey(0xaa), "longnamebob");
    ...
    function test_reserveBaseName_reverts_for_digit_suffixed_reserved_base_label()
        public
    {
        _grantPopFull(ed);
    ...
    function test_reserveBaseName_reverts_when_reserved_label_already_registered()
        public
    {
        ...
        _gatewayRegisterBaseName(
            IDotnsPopController.FullRegistration({
                label: "longnamebob",
                user: ed,
                link: _linkWithLite(LITE_LABEL_A)
            })
        );
    ...
    function test_public_stranger_can_mint_after_claim_clears_reservation()
        public
    {
        _grantPopFull(ed);
        _reservePop(ed, LITE_LABEL_A, _validChatKey(0xaa), "longnamebob");
    ...
    function test_controller_authorised_but_not_gateway_cannot_enter_pop_flow()
        public
    {
    ...
    function _assertEventEmittedOnce(
        Vm.Log[] memory logs,
        bytes32 sig
    ) internal pure {
        uint256 count;
    ...
    function _assertEventNotEmitted(
        Vm.Log[] memory logs,
        bytes32 sig
    ) internal pure {
        for (uint256 i = 0; i < logs.length; i++) {
    ...
    function test_registerBaseName_standalone_succeeds_when_head_is_expired()
        public
    {
        _grantPopFull(ed);
        _reservePop(ed, LITE_LABEL_A, _validChatKey(0x01), BASE_LABEL_A);
        ...
        assertEq(
            IERC721(address(dotnsRegistrar)).ownerOf(
                uint256(_nodeOf(BASE_LABEL_A))
            ),
            tiago
        );
    }

    function test_registerBaseName_standalone_succeeds_when_queue_empty()
        public
    {
        _grantPopFull(ed);
    ...
    function test_registerBaseName_claim_path_bypasses_standalone_holder_guard()
        public
    {
        _grantPopFull(ed);
        _reservePop(ed, LITE_LABEL_A, _validChatKey(0x01), BASE_LABEL_A);
    ...
    function test_registerBaseName_guard_blocks_stranger_and_preserves_claim()
        public
    {
        _grantPopFull(ed);
        _reservePop(ed, LITE_LABEL_A, _validChatKey(0x01), BASE_LABEL_A);
        ...
        _grantPopFull(tiago);
        IDotnsPopController.Link memory strangerLink = _linkFresh(
            _validChatKey(0xbb)
        );
    ...
    function test_reserveBaseName_reserved_label_classification_reverts()
        public
    {
        _grantPopFull(ed);
    ...
    function test_registerBaseName_popFull_user_on_popFull_label_succeeds()
        public
    {
        _grantPopFull(ed);
    ...
    function test_reserveLiteName_succeeds_regardless_of_base_reservation()
        public
    {
        string memory baseStem = BASE_LABEL_A;
        ...
        _gatewayReserveLiteName(
            IDotnsPopController.LiteRegistration({
                liteLabel: "freshli01",
                user: fresh,
                chatKey: _validChatKey(0xcc)
            })
        );

        assertEq(
            IERC721(address(dotnsRegistrar)).ownerOf(
                uint256(_nodeOf("freshli01"))
            ),
            fresh
        );
    }
    ...
    function test_reserveLiteName_reverts_when_suffix_is_not_exactly_two_digits()
        public
    {
        _grantPopFull(ed);
    ...
    function test_reserveLiteName_reverts_when_flattened_label_is_governance_reserved()
        public
    {
        _grantPopFull(ed);
    ...
    function test_reserveBaseName_lite_and_base_legs_both_succeed_in_one_call()
        public
    {
        _grantPopFull(ed);
    ...
    function test_reserveBaseNameOnly_reverts_for_reserved_or_suffixed_labels()
        public
    {
        vm.expectRevert(IDotnsPopController.InvalidBaseLabel.selector);
        _gatewayReserveBaseNameOnly(
        ...
        _gatewayReserveBaseNameOnly(
            IDotnsPopController.BaseNameReservation({
                user: ed,
                reservedBaseLabel: "longnamebob01"
            })
        );
    }

    function test_reserveBaseNameOnly_reverts_when_label_already_registered()
        public
    {
    ...
    function test_reserveBaseNameOnly_same_user_can_replace_prior_reservation()
        public
    {
        _gatewayReserveBaseNameOnly(
            IDotnsPopController.BaseNameReservation({
        ...
        _gatewayReserveBaseNameOnly(
            IDotnsPopController.BaseNameReservation({
                user: ed,
                reservedBaseLabel: BASE_LABEL_B
            })
        );
        ...
        (bool secondReserved, address holder) = dotnsPopController
            .isReservedForClaim(BASE_LABEL_B);
        assertTrue(secondReserved);
        assertEq(holder, ed);
    }
    ...
    function test_user_claimLabelStore_fallback_still_settles_after_gateway_mint()
        public
    {
        _grantPopFull(ed);
        _gatewayReserveLiteName(
    ...
    function test_gateway_can_settle_label_store_for_user() public {
        ...
        _dispatchFromRoot(
            abi.encodeCall(IDotnsPopController.claimLabelStoreFor, (ed))
        );
    ...
    function test_revert_setReservationDuration_below_minimum() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IDotnsPopController.ReservationDurationTooLow.selector,
                0
            )
        );
    ...
    function test_gatewayReserve_stashes_pending_claim_when_user_has_no_label_store()
        public
    {
        _grantPopFull(ed);
        bytes memory chatKey = _validChatKey(0x01);
    ...
    function test_claimLabelStore_deploys_store_and_writes_label_and_chat_key()
        public
    {
        _grantPopFull(ed);
        ...
        bytes32 node = _nodeOf(LITE_LABEL_A);
        assertEq(
            ILabelStore(store).getLabel(node),
    ...
    function test_claimLabelStore_emits_settled_and_name_registered() public {
        ...
        address expectedStore = vm.computeCreateAddress(
            address(storeFactory),
            vm.getNonce(address(storeFactory))
        );

        vm.prank(ed);
        vm.expectEmit(true, true, false, true, address(dotnsPopController));
        emit IDotnsPopController.PendingClaimSettled(
            ed,
            labelhash,
            expectedStore
        );
        vm.expectEmit(true, true, true, true, address(dotnsPopController));
        emit IDotnsPopController.NameRegistered(
            LITE_LABEL_A,
            labelhash,
            ed,
            expectedStore
    ...
    function test_revert_claimLabelStore_when_caller_has_no_pending_claim()
        public
    {
        vm.prank(ed);
        vm.expectRevert(
            abi.encodeWithSelector(
                IDotnsPopController.NoPendingClaim.selector,
                ed
            )
        );
        dotnsPopController.claimLabelStore();
    ...
    function test_revert_claimLabelStore_when_pending_claim_has_lapsed()
        public
    {
        _grantPopFull(ed);
        _gatewayReserveLiteName(
    ...
    function test_expirePendingClaim_clears_entry_after_reservation_duration()
        public
    {
        _grantPopFull(ed);
        _gatewayReserveLiteName(
    ...
    function test_revert_expirePendingClaim_when_user_has_no_pending_claim()
        public
    {
        vm.expectRevert(
            abi.encodeWithSelector(
    ...
    function test_revert_expirePendingClaim_when_entry_is_still_live() public {
        ...
        _gatewayReserveLiteName(
            IDotnsPopController.LiteRegistration({
                liteLabel: LITE_LABEL_A,
                user: ed,
                chatKey: _validChatKey(0x04)
            })
        );

        vm.expectRevert(
            abi.encodeWithSelector(
                IDotnsPopController.PendingClaimNotExpired.selector,
                ed
            )
        );
        dotnsPopController.expirePendingClaim(ed);
    ...
    function test_reserveLiteName_piles_second_pending_claim_when_caller_has_no_store()
        public
    {
        ...
        _gatewayReserveLiteName(
            IDotnsPopController.LiteRegistration({
                liteLabel: LITE_LABEL_A,
                user: ed,
                chatKey: _validChatKey(0x05)
            })
        );
        _gatewayReserveLiteName(
            IDotnsPopController.LiteRegistration({
                liteLabel: LITE_LABEL_B,
                user: ed,
                chatKey: _validChatKey(0x06)
            })
        );
    ...
    function test_pendingClaims_returns_empty_array_for_fresh_user()
        public
        view
    {
        assertEq(dotnsPopController.pendingClaims(ed).length, 0);
    ...
    function test_registerBaseName_claim_by_store_less_full_person_piles_then_settles()
        public
    {
    ...
    function test_pendingClaimUsers_enumeration_mirrors_stash_and_settle()
        public
    {
        _grantPopFull(ed);
        _grantPopFull(tiago);
        ...
        _gatewayReserveLiteName(
            IDotnsPopController.LiteRegistration({
                liteLabel: LITE_LABEL_C,
                user: leonardo,
                chatKey: _validChatKey(0x03)
            })
        );
    ...
    function test_pendingClaimUsers_returns_empty_when_offset_past_count()
        public
    {
        _grantPopFull(ed);
        _gatewayReserveLiteName(
    ...
    function test_claimLabelStore_at_exact_expiry_boundary_belongs_to_user()
        public
    {
        _grantPopFull(ed);
        _gatewayReserveLiteName(
    ...
    function test_claimLabelStore_msg_sender_keyed_other_users_stash_untouched()
        public
    {
        _grantPopFull(ed);
    ...
    function test_gatewayReserve_pending_claim_lapses_after_minimum_duration()
        public
    {
    ...
    function test_claimLabelStore_with_empty_chat_key_skips_resolver_write()
        public
    {
        _grantPopFull(ed);
        _gatewayReserveLiteName(
    ...
    function test_gatewayReserve_warm_user_after_claim_writes_directly_without_stashing()
        public
    {
        _grantPopFull(ed);
        _gatewayReserveLiteName(
    ...
    function test_advanceExpiredHead_promotes_waiter_and_resyncs_popRules()
        public
    {
        ...
        _assertEventEmittedOnce(
            logs,
            keccak256("ReservationExpired(bytes32,address)")
        );
        _assertEventEmittedOnce(
            logs,
            keccak256("ReservationHeadAdvanced(bytes32,address)")
        );
