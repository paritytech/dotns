...
contract DotnsNameEscrowFuzzTest is BaseDotns {
    function testFuzz_deposit_amount(uint256 seed) public {
        ...
        assertEq(
            position.amount,
            requiredPrice,
            "Position amount must match price paid"
        );
        assertEq(
            position.asset,
            address(0),
            "Position asset must be native token"
        );
    ...
    function testFuzz_withdraw_timing(uint256 warpAmount) public {
        ...
        if (warpAmount < ESCROW_COOLDOWN) {
            ...
            assertEq(
                ...
                requiredPrice,
                "Registrant must receive full refund"
            );

            IDotnsNameEscrow.ReleasePosition
                memory positionAfter = dotnsNameEscrow.getReleasePosition(
                    tokenId
                );
            assertEq(
                positionAfter.amount,
                0,
                "Position amount must be zero after withdrawal"
            );
            assertTrue(
                positionAfter.claimed,
                "Position must be marked as claimed"
            );
        }
    }
