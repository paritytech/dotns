...
contract DotnsRegistrarControllerFuzzTest is BaseDotns {
    function testFuzz_owner_setrole_matches_hasrole(
        address account,
        bool enabled
    ) public {
    ...
    function testFuzz_register_pushes_overpayment_back_to_eoa_payer(
        ...
        IDotnsRegistrarController.Registration memory registration = _commitFor(
            nameLabel,
            registrant,
            true
        );

        uint256 requiredPrice = popRules
            .priceWithCheck(nameLabel, registrant)
            .price;
        ...
        dotnsRegistrarController.register{value: requiredPrice + extra}(
            registration
        );
        vm.stopPrank();
    ...
    function testFuzz_register_refunds_overpayment_to_payer_not_owner(
        ...
        uint256 requiredPrice = popRules
            .priceWithCheck(nameLabel, nameOwner)
            .price;
        assertEq(requiredPrice, 0);
        ...
        assertEq(
            payer.balance,
            payerBalanceBefore,
            "EOA payer must net to zero on a free mint"
        );
        assertEq(
    ...
    function testFuzz_transfer_writes_label_to_recipient_store(
        uint256 salt
    ) public {
        address sender = ed;
        ...
        vm.prank(sender);
        dotnsRegistrar.transferFrom{value: _xferFee}(
            sender,
            recipient,
            tokenId
        );
    ...
    function testFuzz_third_party_registration_does_not_overwrite_owner_reverse(
        ...
        IDotnsRegistrarController.Registration
            memory primaryRegistration = _commitFor(
                primaryName,
                nameOwner,
                true
            );
        ...
        IDotnsRegistrarController.Registration
            memory giftedRegistration = _commitFor(
                giftedName,
                nameOwner,
                true,
                payer
    ...
    function testFuzz_NoStatus_transfer_rebinds_position_to_new_holder(
        ...
        uint256 ownerPrice = popRules
            .priceWithCheck(nameLabel, depositor)
            .price;
        assertEq(
            ownerPrice,
            RENT_PRICE,
        ...
        assertEq(
            atMint.amount,
            RENT_PRICE,
            "position must hold full deposit after register"
        );
        assertEq(
            atMint.recipient,
            depositor,
        ...
        uint256 depositorRefundsBefore = dotnsNameEscrow.pendingRefundCount(
            depositor
        );
        uint256 recipientRefundsBefore = dotnsNameEscrow.pendingRefundCount(
            recipientSeed
        );
        ...
        vm.prank(depositor);
        dotnsRegistrar.transferFrom{value: transferFee}(
            depositor,
            recipientSeed,
            tokenId
        );
        ...
        assertEq(
            ...
            recipientSeed,
            "NFT must move to the recipient"
        );
    ...
    function testFuzz_transfer_clears_sender_primary_reverse(
        uint256 salt
    ) public {
        address sender = ed;
        address recipient = leonardo;
        ...
        assertEq(
            dotnsReverseResolver.nameOf(sender),
            string.concat(nameLabel, ".dot")
        );
    ...
    function _labelNoStatusPriced(
        uint256 salt
    ) internal pure returns (string memory label) {
        return
            string(
                abi.encodePacked("nostatus", _uintToAlphaFixed(salt, 2), "01")
            );
    }

    /// @notice Generate a label that classifies as PopLite and prices to zero.
    function _labelPriceZero(
        uint256 salt
    ) internal pure returns (string memory label) {
        return
            string(abi.encodePacked("free", _uintToAlphaFixed(salt, 2), "01"));
    }
