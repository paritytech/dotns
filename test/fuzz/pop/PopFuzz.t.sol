...
contract PopRulesFuzzTest is BaseDotns {
    function testFuzz_popfull_user_can_access_poplite(
        uint256 seed,
        uint256 length
    ) public {
        length = bound(length, 6, 8);
        string memory nameLabel = string(
            abi.encodePacked(_makeAlpha(seed, length), "01")
        );
    ...
    function testFuzz_popfull_user_can_access_nostatus(
        uint256 seed,
        uint256 length
    ) public {
    ...
    function testFuzz_governance_names_always_revert(
        ...
        IPopRules.PopStatus userStatus = IPopRules.PopStatus(
            bound(statusSeed, 0, 3)
        );
    ...
    function testFuzz_reservation_blocks_other_users(uint256 seed) public {
        ...
        (bool isReserved, address reservedFor, ) = popRules.isBaseNameReserved(
            baseName
        );
    ...
    function testFuzz_price_without_check_returns_price(
        uint256 seed,
        uint256 length
    ) public view {
        ...
        IPopRules.PriceWithMeta memory priceMetadata = popRules
            .priceWithoutCheck(nameLabel, ed);

        assertEq(priceMetadata.price, popRules.price(nameLabel));
    }

    function testFuzz_expired_reservation_rolls_forward_to_next_lite_registrant(
        uint256 seed
    ) public {
        string memory baseName = _makeAlpha(seed, 6);
        ...
        (bool isReserved, address firstOwner, uint64 firstExpiry) = popRules
    ...
    function testFuzz_mixed_case_names_are_rejected(uint256 seed) public {
        ...
        string memory mixedCaseName = string(
            abi.encodePacked(string(baseName), "01")
        );
    ...
    function _makeAlpha(
        ...
        for (uint256 i = 0; i < length; i++) {
            buffer[i] = bytes1(
                uint8(97 + (uint256(keccak256(abi.encodePacked(seed, i))) % 26))
            );
        }
