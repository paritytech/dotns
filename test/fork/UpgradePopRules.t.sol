...
contract UpgradePopRulesForkTest is Test {
    ...
    function test_upgrade_rootOpensShortNameMarket() public {
        ...
        // While the market is closed the public preview rejects a short label. Assert this only
        // when it is actually closed on live state, so a governance toggle already flipped on the
        // fork
        // does not fail the transition this test exercises.
        if (!popRules.shortNamesEnabled()) {
            vm.expectRevert(
