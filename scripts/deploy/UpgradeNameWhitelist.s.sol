...
/// @dev The swap changes only `initialize`, which does not run on an upgrade. It is swapped for
/// parity, so the deployment is one release throughout rather than one contract behind.
///
...
contract UpgradeNameWhitelist is BaseDeployer {
    ...
    /// @notice Manifest label the DotnsNameWhitelist proxy is recorded under.
    string internal constant NAME_WHITELIST_LABEL = "DotnsNameWhitelist";

    /// @notice Reads the manifest, resolves the DotnsNameWhitelist proxy, and upgrades it as
    /// `msg.sender`.
    ...
    function run() external {
        ...
        address proxy = _readAddress(NAME_WHITELIST_LABEL);
    ...
    /// @param proxy DotnsNameWhitelist proxy address resolved from the manifest.
    function _upgradeNameWhitelist(address owner, address proxy) internal {
        require(
            owner == OwnableUpgradeable(proxy).owner(),
            "UpgradeNameWhitelist: broadcaster is not the proxy owner"
        );
