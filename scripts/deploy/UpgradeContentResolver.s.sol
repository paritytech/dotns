...
/// @title UpgradeContentResolver
/// @notice Upgrades the deployed DotnsContentResolver proxy to the current implementation. Resolves
/// the proxy from the on-disk manifest, diffs the new storage layout against the pinned
///         @custom:contract DotnsContentResolverOld snapshot, and swaps the implementation only
...
contract UpgradeContentResolver is BaseDeployer {
    ...
    /// @notice Manifest label the DotnsContentResolver proxy is recorded under.
    string internal constant CONTENT_RESOLVER_LABEL = "DotnsContentResolver";

    /// @notice Reads the manifest, resolves the DotnsContentResolver proxy, and upgrades it as
    /// `msg.sender`.
    ...
    function run() external {
        ...
        address proxy = _readAddress(CONTENT_RESOLVER_LABEL);
        _upgradeContentResolver(owner, proxy);

        console.log("=== UpgradeContentResolver complete ===");
    }

    /// @notice Upgrades `proxy` to the current `DotnsContentResolver` implementation under `owner`.
    ...
    /// @param proxy DotnsContentResolver proxy address resolved from the manifest.
    function _upgradeContentResolver(address owner, address proxy) internal {
        require(
            owner == OwnableUpgradeable(proxy).owner(),
            "UpgradeContentResolver: broadcaster is not the proxy owner"
        );
        ...
        console.log("  upgraded DotnsContentResolver proxy", proxy);
    }
}
