...
/// @notice Upgrades the deployed DotnsResolver proxy to the current implementation. Resolves the
///         proxy from the on-disk manifest, diffs the new storage layout against the pinned
///         @custom:contract DotnsResolverOld snapshot, and swaps the implementation only when the
...
contract UpgradeResolver is BaseDeployer {
    ...
    string internal constant REFERENCE_CONTRACT = "DotnsResolverOld.sol:DotnsResolverOld";

    /// @notice Manifest label the DotnsResolver proxy is recorded under.
    string internal constant RESOLVER_LABEL = "DotnsResolver";

    /// @notice Reads the manifest, resolves the DotnsResolver proxy, and upgrades it as
    /// `msg.sender`.
    ...
    function run() external {
        ...
        address proxy = _readAddress(RESOLVER_LABEL);
        _upgradeResolver(owner, proxy);

        console.log("=== UpgradeResolver complete ===");
    }

    /// @notice Upgrades `proxy` to the current `DotnsResolver` implementation under `owner`.
    ...
    /// @param proxy DotnsResolver proxy address resolved from the manifest.
    function _upgradeResolver(address owner, address proxy) internal {
        require(
            owner == OwnableUpgradeable(proxy).owner(),
            "UpgradeResolver: broadcaster is not the proxy owner"
        );
        ...
        console.log("  upgraded DotnsResolver proxy", proxy);
    }
}
