...
`deployments/<network>/<chainId>.json` is a **network record**: what is deployed on that live network right now. It is updated only by a real deploy or migration on that network, never by a code change. Nothing else value-checks these files, since the CREATE3 parity gates compare `expected.json` against a fresh CI deploy, so the rule is enforced in CI instead: a pull request editing one fails `release-metadata.yml` unless it carries the `deployment-record` label. Everything that answers for reality reads these files: releases copy their addresses verbatim, and pointing tooling or the wire stage at an address with nothing behind it breaks whatever reads it.
